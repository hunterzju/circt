...
Attribute AttributeAttr::parse(DialectAsmParser &p, Type type) {
  ...
  if (p.parseGreater())
    return Attribute();
  return AttributeAttr::get(p.getContext(), name, expression);
}

void AttributeAttr::print(::mlir::DialectAsmPrinter &p) const {
